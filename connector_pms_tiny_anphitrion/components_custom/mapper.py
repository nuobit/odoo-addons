...
class ImportMapChild(AbstractComponent):
    ...
    def classify_items(self, mapped, to_attr, options):
        def keygen(item):
            binder = self.binder_for()
            return tuple(binder.dict2id(item))

        binding = options["binding"]
        if binding:
            old = {}
            for rec in options["binding"][to_attr]:
                old.setdefault(keygen(rec), []).append(rec.id)
            new = {}
            for value in mapped:
            ...
            for key in set(old.keys()) & set(new.keys()):
                for _id, value in zip_longest(old[key], new[key]):
                    # update
                    if value and _id:
