...
class BinderComposite(AbstractComponent):
    ...
    def idhash(self, external_id):
        ...
        for e in external_id:
            if isinstance(e, int):
                e9 = str(e)
                if int(e9) != e:
                    raise ValidationError(_("Unexpected"))
            elif isinstance(e, str):
                e9 = e
            elif e is None:
                pass
            else:
                raise ValidationError(_("Unexpected type for a key: type %s") % type(e))
            odoo_hash.update(e9.encode("utf8"))
        return odoo_hash.hexdigest()

    def _idfields2list(self, _id):
    ...
    def idfields(self, in_field=True):
        field = in_field and self._internal_field or self._external_field
        return self._idfields2list(field)
    ...
    def id2dict(self, _id, in_field=True):
        ...
        """
        _id = self._idfields2list(_id)
        field = self.idfields(in_field=in_field)
    ...
    def _find_binding(self, relation, binding_extra_vals=None):
        if binding_extra_vals is None:
            binding_extra_vals = {}
        if self._is_binding(relation):
            raise ValidationError(
                _("The source object %s must not be a binding") % relation.model._name
            )
    ...
    def wrap_binding(self, relation, binding_field=None, binding_extra_vals=None):
        ...
        if binding_field is None:
            if not self._binding_field:
                raise ValidationError(
                    _("_binding_field defined on synchronizer class is mandatory")
                )
        ...
        if not self._is_binding(binding):
            raise ValidationError(
                _("Expected binding '%s' and found regular model '%s'")
        ...
        if isinstance(binding, models.BaseModel):
            ...
            if wrap:
                if not wrapped_model:
                    raise ValidationError(
                        _("The wrapped model is mandatory if binding is not an object")
                    )
    ...
    def _check_domain(self, domain):
        for field, _unused, value in domain:
