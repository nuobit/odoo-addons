...
class GenericExporterCustom(AbstractComponent):
    ...
    def run(self, relation, internal_fields=None):
        ...
        if self.binding:
            ...
            # values = self._update_data(map_record, fields=None, **opts)
            if not values:
                result = _("Nothing to export")
            ...
            self.binder.bind_export(external_data, relation)

        result = _("Record exported with ID %s on Backend.") % 'external_id'

        # result = self._run(*args, **kwargs)

        # self.binder.bind(self.external_id, relation, export=True)
