...
class SaleOrderWPMLImportMapChild(Component):
    _name = "woocommerce.wpml.sale.order.map.child.import"
    _inherit = "woocommerce.wpml.map.child.import"

    _apply_on = "woocommerce.wpml.sale.order.line"

    def get_item_values(self, map_record, to_attr, options):
        binder = self.binder_for("woocommerce.wpml.sale.order.line")
...
class WooCommerceWPMLSaleOrderImportMapper(Component):
    _name = "woocommerce.wpml.sale.order.import.mapper"
    ...
    children = [
        (
            "line_items",
            "woocommerce_wpml_order_line_ids",
            "woocommerce.wpml.sale.order.line",
        )
    ]
    ...
    def payment_method(self, record):
        payment_mode = self.env["woocommerce.wpml.backend.payment.mode"]
    ...
    @mapping
    def woocommerce_wpml_order_id(self, record):
        client_order_ref = (
    ...
    def status(self, record):
        return {"woocommerce_wpml_status": record["status"]}
