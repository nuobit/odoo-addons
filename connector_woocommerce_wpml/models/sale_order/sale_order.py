...
class SaleOrder(models.Model):
    ...
    woocommerce_wpml_status_write_date = fields.Datetime(
        compute="_compute_woocommerce_wpml_status_write_date",
        store=True,
    )

    @api.depends("state", "picking_ids", "picking_ids.state")
    def _compute_woocommerce_wpml_status_write_date(self):
        for rec in self:
            if rec.is_woocommerce:
                rec.woocommerce_wpml_status_write_date = fields.Datetime.now()
    ...
    @api.depends(
        ...
        "woocommerce_wpml_bind_ids",
        "picking_ids.woocommerce_wpml_stock_picking_state",
        "picking_ids.state",
    )
    def _compute_woocommerce_order_state(self):
        for rec in self:
            if rec.is_woocommerce:
                # TODO: What to do if there are no pickings?
                if self.picking_ids:  # Added this line
    ...
    def action_confirm(self):
        ...
        if self.woocommerce_wpml_bind_ids.woocommerce_wpml_status == "on-hold":
            self._event("on_compute_woocommerce_order_state").notify(
