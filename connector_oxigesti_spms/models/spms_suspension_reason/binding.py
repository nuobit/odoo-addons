...
class SPMSSuspensionReasonBinding(models.Model):
    _name = "oxigesti.spms.spms.suspension.reason"
    _description = "Oxigesti SPMS SPMS Suspension Reason Binding"

    _inherit = "oxigesti.spms.binding"
    _inherits = {"spms.suspension.reason": "odoo_id"}

    odoo_id = fields.Many2one(
        comodel_name="spms.suspension.reason",
        string="Odoo ID",
        required=True,
        ondelete="cascade",
    )
    ...
    @api.model
    def import_spms_suspension_reason_since(self, backend_record, since_date=None):
        domain = []
        # if since_date:
    ...
    @api.model
    def export_spms_suspension_reason_since(self, backend_record, since_date=None):
