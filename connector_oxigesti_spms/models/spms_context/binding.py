...
class SPMSContextBinding(models.Model):
    _name = "oxigesti.spms.spms.context"
    _description = "Oxigesti SPMS SPMS Context Binding"

    _inherit = "oxigesti.spms.binding"
    _inherits = {"spms.context": "odoo_id"}

    odoo_id = fields.Many2one(
        comodel_name="spms.context",
        string="Odoo ID",
        required=True,
        ondelete="cascade",
    )
    ...
    @api.model
    def import_spms_context_since(self, backend_record, since_date=None):
        domain = []
        # if since_date:
    ...
    @api.model
    def export_spms_context_since(self, backend_record, since_date=None):
