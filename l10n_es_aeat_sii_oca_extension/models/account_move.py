    ...
    if isinstance(source_dict, (str | int | float | bool)):
        if get_path_value(target_dict, path) != source_dict:
            raise ValidationError(
                _(
                    "SII data merge error: No match at path %(path)s:"
                    " %(target_value)s != %(source_value)s"
                )
                % {
                    "path": path,
                    "target_value": get_path_value(target_dict, path),
        ...
        raise UserError(
            _("SII data merge error: Unsupported type: %(type)s: %(value)s")
            % {"type": type(source_dict), "value": source_dict}
        )
...
def merge_detalle_iva(tax_details_list1, tax_details_list2):
    ...
    for tax_detail in tax_details_list1 + tax_details_list2:
        if rate_key not in tax_detail:
            raise ValidationError(
                _("SII tax detail merge error: Missing '%(key)s' key in %(detail)s")
                % {"key": rate_key, "detail": tax_detail}
            )
        ...
        if tax_rate not in merged_by_rate:
            ...
            for key, value in tax_detail.items():
                if key not in merged_by_rate[tax_rate]:
                    raise ValidationError(
                        _(
                            "SII tax detail merge error:"
                            " Missing '%(key)s' key in %(detail)s"
                        )
                        % {"key": key, "detail": merged_by_rate[tax_rate]}
                    )
...
class AccountMove(models.Model):
    ...
    def _get_aeat_invoice_dict(self):
        inv_dict = super()._get_aeat_invoice_dict()
        round_by_keys(
            inv_dict,
            [
                "BaseImponibleACoste",
