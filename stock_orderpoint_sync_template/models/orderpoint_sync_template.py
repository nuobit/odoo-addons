...
class OrderpointSyncTemplate(models.Model):
    ...
    location_ids = fields.Many2many(string='Locations', comodel_name='stock.location',
                                    relation='stock_warehouse_orderpoint_sync_template_location_rel',
                                    column1='template_id', column2='location_id',
                                    required=True)
    route_ids = fields.Many2many(string='Routes', comodel_name='stock.location.route',
                                 relation='stock_warehouse_orderpoint_sync_template_route_rel',
                                 column1='template_id', column2='route_id',
                                 required=False)
    ...
    line_ids = fields.One2many(comodel_name='stock.warehouse.orderpoint.sync.template.line',
                               inverse_name='sync_template_id', copy=True)
    ...
    def create_orderpoints(self):
        ...
        for rec in self:
            # orderpoints
            created, updated, deleted = 0, 0, 0
            for l in rec.location_ids:
                crt, upd, dele = generate_orderpoint(rec, l)
                created += crt
                updated += upd
                deleted += dele
            ...
            rec.status = _("Orderpoints: Created: %i, Updated: %i, Deleted: %i | "
                           "Product routes: Routes added: %i, Products updated: %i") % (
                             created, updated, deleted, radded, pupdated)

    _sql_constraints = [('namecomp_uniq', 'unique (company_id, name)', 'Name must be unique per company!'),
                        ]
...
class OrderpointSyncTemplateLine(models.Model):
    ...
    orderpoint_ids = fields.One2many(comodel_name='stock.warehouse.orderpoint',
                                     inverse_name='sync_template_line_id',
                                     string='Orderpoints', copy=True)

    sync_template_id = fields.Many2one(comodel_name='stock.warehouse.orderpoint.sync.template',
                                       ondelete='cascade')

    _sql_constraints = [('prodtemp_uniq', 'unique (sync_template_id, product_id)',
