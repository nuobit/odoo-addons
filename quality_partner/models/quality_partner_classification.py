...
LEVELS = [(k, trl) for k, (ord, trl) in sorted(MAP_LEVELS.items(), key=lambda x: x[1][0])]
...
class QualityPartnerClassification(models.Model):
    ...
    parent_id = fields.Many2one(comodel_name='quality.partner.classification', string='Parent',
                                on_delete='restrict')

    document_type_ids = fields.Many2many(comodel_name='quality.partner.document.type',
                                         relation='quality_partner_classification_document_type_rel',
                                         column1='classification_id', column2='document_type_id',
                                         string="Document types")

    sequence = fields.Integer(string='Sequence')

    mandatory_document_type_ids = fields.Many2many(comodel_name='quality.partner.document.type',
    ...
    def _compute_mandatory_document_type_ids(self):
        for rec in self:
            rec.mandatory_document_type_ids = \
                rec._get_document_type_ids()
    ...
    def _check_parent_id(self):
        objs = self.env[self._name].search([]).sorted(
            lambda x: (x._get_root().id, x.level, x.name))
        ...
        for rec in self:
            if rec.level_type != 'criticality':
                ...
                if not rec.document_type_ids:
                    raise ValidationError(_("You must provide a document types for the level type selected"))

    @api.constrains('level_type', 'parent_id', 'parent_id.level_type')
    def _check_level_type_parent(self):
        for rec in self:
            if rec.parent_id == rec:
                raise ValidationError(_("The parent cannot be the same as current record"))
            if not ((not rec.parent_id and rec.level_type == MAP_LEVELS_ORD[0][0]) or \
                    (rec.parent_id and MAP_LEVELS[rec.level_type][0] == MAP_LEVELS[rec.parent_id.level_type][0] + 1)):
                raise ValidationError(_("Wrong inheritance, the level type selected it's not "
