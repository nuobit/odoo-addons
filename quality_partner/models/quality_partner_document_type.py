...
class QualityPartnerDocumentType(models.Model):
    ...
    @api.multi
    def unlink(self):
        records = self.env['quality.partner.classification'].search([('document_type_ids', 'in', self.ids)])
        if records:
            raise UserError(_("You are trying to delete a record that is still referenced!"))
        records = self.env['quality.partner.document'].search([('document_type_id', 'in', self.ids)])
        if records:
            raise UserError(_("You are trying to delete a record that is still referenced!"))
