...
class ComputeVatProrrate(models.TransientModel):
    ...
    def button_compute(self):
        ...
        if self.env['aeat.map.special.prorrate.year'].search([('year', '=', next_year)]):
            raise UserError(_("The prorrate is already computed for this year"))
        ...
        mod303.vat_prorrate_percent = prorrate_percent

        self.env['aeat.map.special.prorrate.year'].create({
            'year': next_year,
