...
class L10nEsAeatMod303Report(models.Model):
    ...
    def calculate(self):
        ...
        child_taxes = AccountTax.search(
            [('prorrate_type', '!=', False),
             ('company_id', 'child_of', self.company_id.id)])
        # parent_taxes = AccountTax.search([
        #     ('amount_type', '=', 'group'),
        #     ('children_tax_ids', 'in', child_taxes),
        # ])
        ...
        move_line_domain.append(('tax_line_id', 'in', child_taxes.ids))

        move_lines = self.env['account.move.line'].search(move_line_domain)

        key_fields = ['move_id', 'account_id', 'partner_id', 'name',
                      'tax_line_id', 'tax_exigible']
        move_lines_grouped = {}
        for line in move_lines:
            key = tuple([getattr(line, f) for f in key_fields])
            move_lines_grouped.setdefault(key, {
                'credit': 0,
                'debit': 0,
            })
            move_lines_grouped[key].credit += line.credit
            move_lines_grouped[key].debit += line.debit



        move_lines_by_move = self.env['account.move.line'].read_group(
            move_line_domain, ['move_id', 'id', 'account_id', 'partner_id', 'name',
                               'tax_line_id', 'tax_exigible', 'credit', 'debit'],
            ['move_id'],
        )
        ...
        for report in self:
            report.casilla_44 = 0
            if (report.vat_prorrate_type not in ('general', 'special') or
    ...
    def _prepare_tax_line_vals(self, map_line):
        res = super(L10nEsAeatMod303Report, self)._prepare_tax_line_vals(
            map_line)
        if (self.vat_prorrate_type in ('general', 'special') and
    ...
    def _process_tax_line_regularization(self, tax_lines):
        ...
        for tax_line in tax_lines:
            ...
            all_lines += lines
            if (self.vat_prorrate_type not in ('general', 'special') or
