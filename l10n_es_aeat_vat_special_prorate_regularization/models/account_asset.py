...
class L10nEsAeatMod303Report(models.AbstractModel):
    ...
    def _default_counterpart_capital_assets_receivable_account_id(self):
        return self.get_account_from_template(self.env.ref('l10n_es.account_common_6342'))
    ...
    def _default_counterpart_capital_assets_payable_account_id(self):
        return self.get_account_from_template(self.env.ref('l10n_es.account_common_6392'))
...
class AccountAsset(models.Model):
    ...
    def _compute_tax_percentage(self):
        for rec in self:
            percentage_line = rec.env['aeat.map.special.prorrate.year'].get_by_ukey(rec.company_id.id,
                                                                                    rec.start_date_use.year)
            rec.temp_prorrate_percent = percentage_line.tax_percentage
            rec.final_prorrate_percent = percentage_line.tax_final_percentage

    # iva_ded_prov = fields.Float(string="IVA Deducible Provisional", compute="_compute_iva_ded_prov")
    #
    # def _compute_iva_ded_prov(self):
    #     for rec in self:
    #         prorate_line = rec.env['aeat.map.special.prorrate.year'].get_by_ukey(rec.company_id.id, rec.date.year)
    #         rec.amount_currency = rec.move_line_id.amount_currency
