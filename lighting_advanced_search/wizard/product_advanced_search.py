...
class LightingProductAdvancedSearch(models.TransientModel):
    ...
    # attachment search
    attachment_type_in_ids = fields.Many2many(comodel_name='lighting.attachment.type',
                                   relation='lighting_product_advanced_search_attachment_type_in_rel',
                                   column1='advanced_search_id',
                                   column2='type_id')
    attachment_type_in_op = fields.Selection(selection=OP_SEL, string='Operator', default='and')
    attachment_type_not_in_ids = fields.Many2many(comodel_name='lighting.attachment.type',
                                       relation='lighting_product_advanced_search_attachment_type_not_in_rel',
                                       column1='advanced_search_id',
                                       column2='type_id')
    attachment_type_not_in_op = fields.Selection(selection=OP_SEL, string='Operator', default='and')
    ...
    def advanced_search(self):
        ...
        if self.attachment_type_in_ids:
            for type_id in self.attachment_type_in_ids:
        ...
        if self.attachment_type_not_in_ids:
            domain_not_in_tmp = []
            for type_id in self.attachment_type_not_in_ids:
