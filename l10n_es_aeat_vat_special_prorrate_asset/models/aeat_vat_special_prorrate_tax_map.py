...
class AEATVatspecialProrrateTaxMap(models.Model):
    ...
    src_tax_id = fields.Many2one(
        ...
        ondelete="restrict",
        required=True,
        string="Source tax code",
    )

    src_tax_name = fields.Char(
        related="src_tax_id.name",
        readonly=True,
        comodel_name="account.tax.template",
        ondelete="restrict",
        string="Source tax",
    ...
    tgt_tax_id = fields.Many2one(
        ...
        ondelete="restrict",
        required=True,
        string="Target tax code",
    )

    tgt_tax_name = fields.Char(
        related="tgt_tax_id.name",
        readonly=True,
        comodel_name="account.tax.template",
        ondelete="restrict",
        string="Target tax",
    ...
    _sql_constraints = [
        ("unique_tax_map", "unique(src_tax_id, tgt_tax_id)", "Tax map must be unique"),
    ]
