...
class L10nEsAeatMod303ProrrateReport(models.Model):
    ...
    parent_id = fields.Many2one(
        comodel_name="l10n.es.aeat.mod303.report", ondelete="cascade", required=True
    )

    @api.model
    def get_parent_context(self):
        return self.parent_id.browse(self.env.context.get("default_parent_id"))
    ...
    state = fields.Selection(
        ...
        string="Prorrate state",
        default="final_prorrate",
        readonly=True,
    )
    ...
    def _default_map_prorrate_year_id(self):
        ...
        map_prorrate = self.map_prorrate_year_id.get_by_ukey(
            parent.company_id.id, parent.year
        )
        if not map_prorrate:
            raise ValidationError(
                _("Prorrate record not found for year %i") % parent.year
            )
        return map_prorrate

    map_prorrate_year_id = fields.Many2one(
    ...
    temp_prorrate_percent = fields.Float(
        related="map_prorrate_year_id.tax_percentage",
        string="Temporary prorrate (%)",
        readonly=True,
    )
    final_prorrate_percent = fields.Float(
        related="map_prorrate_year_id.tax_final_percentage",
    ...
    first_reg_positive_adjust_account_id = fields.Many2one(
        comodel_name="account.account",
        string="Positive adjustment account",
        required=True,
        domain=[("deprecated", "=", False)],
    )
    first_reg_negative_adjust_account_id = fields.Many2one(
        comodel_name="account.account",
        string="Negative adjustment account",
        required=True,
        domain=[("deprecated", "=", False)],
    )
    first_reg_ref = fields.Char(
        string="Reference", default="Regularización prorrata definitiva", required=True
    )
    first_reg_move_id = fields.Many2one(
    ...
    second_reg_positive_adjust_account_id = fields.Many2one(
        comodel_name="account.account",
        string="Positive adjustment account",
        required=True,
        domain=[("deprecated", "=", False)],
    )
    second_reg_negative_adjust_account_id = fields.Many2one(
        comodel_name="account.account",
        string="Negative adjustment account",
        required=True,
        domain=[("deprecated", "=", False)],
    )
    second_reg_ref = fields.Char(
        string="Reference",
        default="Regularización bienes de inversión prorrata",
        required=True,
    )
    second_reg_move_id = fields.Many2one(
        comodel_name="account.move",
        string="Journal Entry",
        readonly=True,
        index=True,
    ...
    def _check_date(self):
        for rec in self:
            ...
            if move_date.year != self.parent_id.year:
                raise ValidationError(
                    _(
                        "The year of the move date should be the same as the year we are dealing with"
                    )
                )
    ...
    def find_invoice_line(self, line, base, parent_tax):
        ...
        cnd = invoice_line_ids.filtered(lambda x: parent_tax in x.invoice_line_tax_ids)
        if len(cnd) == 1:
            return cnd

        cnd2 = cnd.filtered(lambda x: abs(x.price_subtotal_signed - base) < 0.02)

        if not cnd2:
            raise ValidationError(
                _("The invoice line cannot be inferred from %s") % cnd.mapped("name")
            )
        if len(cnd2) > 1:
    ...
    def get_asset(self, invoice_line_id):
        ...
        if not asset:
            raise ValidationError(
                _("No exists any asset on invoice %s with category %s")
                % (invoice_line_id.invoice_id.number, invoice_line_id.asset_category_id)
            )
        if len(asset) > 1:
            raise ValidationError(
                _("More than one asset found with invoice %s and category %s")
                % (invoice_line_id.invoice_id.number, invoice_line_id.asset_category_id)
            )
    ...
    def button_first_regularization_final_prorrate(self):
        ...
        for line in move_lines:
            ...
            new_prorrated_tax_amount = round_curr(
                tax_amount * self.final_prorrate_percent / 100
            )
            ...
            values_adj.update(
                {
                    "account_id": diff > 0
                    and self.first_reg_positive_adjust_account_id.id
                    or self.first_reg_negative_adjust_account_id.id,
            ...
            values_adj.update(
                {
                    "tax_ids": [
                        (6, 0, [parent_tax.id] + parent_tax.children_tax_ids.ids)
                    ],
                }
            )
