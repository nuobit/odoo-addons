...
class AccountBankStatementImport(models.TransientModel):
    ...
    def _complete_stmts_vals(self, stmts_vals, journal, account_number):
        ...
        if self.check_dates:
            ...
            for bs in self.env['account.bank.statement'].search([('journal_id', '=', journal.id)]):
                ...
                if bs_max_date >= imp_min_date and bs_min_date <= imp_max_date:
                    ...
                    raise ValidationError(_("Imported file overlaps with existing Bank statement: %s") % ' '.join(bs_display_name))
