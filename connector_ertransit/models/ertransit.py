    ...
    def _update_javax_faces_viewstate(self, res):
        ...
        for t in tag_javax:
            if 'value' not in t.attrib:
                raise Exception("Unexpected, the javax.faces.ViewState element if found must have value attribute")
            if not self.javax_face_viewstate:
                self.javax_face_viewstate = t.attrib['value']
            else:
                if self.javax_face_viewstate != t.attrib['value']:
                    raise Exception("Unexpected! all the javax.faces.ViewState must have the same 'value'")
