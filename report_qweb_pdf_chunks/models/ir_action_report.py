...
try:
    from PyPDF2 import PdfFileReader, PdfFileMerger  # pylint: disable=W0404
except ImportError:
...
class Report(models.Model):
    ...
    def render_qweb_pdf(self, res_ids=None, data=None):
        ...
        chunk_threshold = int(ir_config.get_param('qweb.report.pdf.chunk.threshold', 50))
        chunk_size = int(ir_config.get_param('qweb.report.pdf.chunk.size', 50))

        if not res_ids or chunk_threshold < 0 or \
                len(res_ids) <= chunk_threshold or len(res_ids) <= chunk_size:
            pdf_content, _ = super(Report, self).render_qweb_pdf(res_ids=res_ids, data=data)
        else:
            chunk_count = ceil(len(res_ids) / chunk_size)
            _logger.info("Many documents to print, splitting in %i chunks of %i (max) "
            ...
            for i, res_ids_chunk in enumerate(chunks(res_ids, chunk_size), 1):
                ...
                pdf_content_chunk, _ = super(Report, self) \
                    .render_qweb_pdf(res_ids=res_ids_chunk, data=data)
