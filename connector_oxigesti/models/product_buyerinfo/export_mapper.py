...
class ProductBuyerExportMapper(Component):
    ...
    def CodigoArticulo(self, record):
        ...
        assert external_id, (
            ...
            f"Oxigesti product, so the Oxigesti ID cannot be obtained. "
            f"At this stage, the Oxigesti product should have been linked via "
            f"ProductProduct._export_dependencies. "
            f"If not, then this product {product_id} "
            f"{product_id.display_name} with code '{product_id.default_code}'"
