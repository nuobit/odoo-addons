...
class ProductCategoryBinder(Component):
    ...
    def _get_external_id(self, binding):
        ...
        adapter = self.component(usage='backend.adapter',
                                 model_name='oxigesti.product.category')
        external_ids = adapter.search([
            ('IdCategoriaOdoo', '=', binding.odoo_id.id)
        ])
        if not external_ids:
            return None
        if len(external_ids) > 1:
            raise ValidationError("More than one Category with ID '%i' on the backend" % (binding.odoo_id.id,))
