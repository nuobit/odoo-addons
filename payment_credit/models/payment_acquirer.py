...
class credit_payment_acquirer(osv.Model):
    _inherit = 'payment.acquirer'

    def _get_providers(self, cr, uid, context=None):
        providers = super(credit_payment_acquirer, self)._get_providers(cr, uid, context=context)
        providers.append(['credit', _('Credit')])
        return providers

    def credit_get_form_action_url(self, cr, uid, id, context=None):
        return '/payment/credit/feedback'

    def _format_credit_data(self, cr, uid, context=None):
...
<p>Please use the order name as communication reference.</p>
    ...
    def create(self, cr, uid, values, context=None):
        ...
        if values.get('provider') == 'credit' and not values.get('post_msg'):
            values['post_msg'] = self._format_credit_data(cr, uid, context=context)
        return super(credit_payment_acquirer, self).create(cr, uid, values, context=context)


class credit_payment_transaction(osv.Model):
    _inherit = 'payment.transaction'

    def _credit_form_get_tx_from_data(self, cr, uid, data, context=None):
    ...
    def _credit_form_get_invalid_parameters(self, cr, uid, tx, data, context=None):
        invalid_parameters = []
    ...
    def _credit_form_validate(self, cr, uid, tx, data, context=None):
        _logger.info('Validated credit payment for tx %s: set as done' % (tx.reference))
