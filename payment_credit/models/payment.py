...
class CreditPaymentAcquirer(models.Model):
    ...
    provider = fields.Selection(selection_add=[('credit', 'Credit')])
...
<p>Please use the order name as communication reference.</p>
    ...
    def create(self, values):
        ...
        if values.get('provider') == 'credit' and not values.get('post_msg'):
            values['post_msg'] = self._format_credit_data()
...
class CreditPaymentTransaction(models.Model):
    ...
    def _credit_form_validate(self, data):
        _logger.info('Validated credit payment for tx %s: set as done' % (self.reference))
